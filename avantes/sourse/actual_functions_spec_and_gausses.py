...
def gauss_for_gists(x, A, sigma, center):
...
def init_data_gist_each_point(data_directory_1, data_directory_2):
    data_blue = avaread.read_file(data_directory_1)
    waves_blue = data_blue.wavelength
    errors_blue = []
    for i in range(len(data_blue)):
        errors_blue.append(data_blue.scope.T[i])
    ...
    return errors_blue, errors_red, waves_blue, waves_red

def init_graph_gist_feach(data_directory_1, data_directory_2):
    errors_blue, errors_red, waves_blue, waves_red = init_data_gist_each_point(data_directory_1, data_directory_2)
...
def init_graph_gist_gauss(data_directory_1, data_directory_2):
    ...
    pixels_list_b = [17, 78, 107, 108, 124, 895, 922, 1588, 1598, 2038, 2042]
    pixels_list_r = [17, 78, 107, 108, 124, 895, 922, 1588, 1598, 2038, 2042]
    ...
    if errors_red != 0:
        ...
        if len(pixels_list_r) > 6:
            cols = 3
        else:
            cols = 2
        rows = int(np.ceil(len(pixels_list_r)/cols))
        ...
        for n, ax in zip(pixels_list_r, axes):
            ...
            bins = [(bins[i]+bins[i+1])/2 for i in range(0, len(bins)-1, 1)]
            bins = np.array(bins)

            idxs = np.argsort(counts)[-5:][::-1]  # индексы N максимальных в порядке убывания
            center_1 = bins[idxs]
            center_1 = np.mean(center_1)

            A_1 = np.mean(counts[idxs])
            sigma_1 = np.std(errors_red[n])

            A, sigma, center = A_1, sigma_1, center_1

            popt, pcov = curve_fit(
                gauss_for_gists, bins, counts,
                p0=[A, sigma, center]
                # bounds=([80, 0, -300],[200, 300, 300])
            )

            x_fit = np.linspace(bins.min(), bins.max(), 500)
            y_fit = gauss_for_gists(x_fit, *popt)
...
def init_data_from_csv(data_directory):
    df = pd.read_csv(data_directory, sep=';', skiprows=5, header=0)

    wavelenght = df.iloc[:, 0].to_numpy()
    data = df.iloc[:, 1:].to_numpy()

    return data, wavelenght

def coef_absolute_to_acp(data_directory_1, data_directory_2, data_directory_3, data_directory_4):
    errors_blue, errors_red, waves_blue, waves_red = init_data_gist_each_point(data_directory_1, data_directory_2)
    errors_blue_abs, waves_blue_abs = init_data_from_csv(data_directory_3)
    errors_red_abs, waves_red_abs = init_data_from_csv(data_directory_4)

    errors_blue_abs = np.transpose(errors_blue_abs)
    errors_red_abs = np.transpose(errors_red_abs)

    coefs_blue = np.array(errors_blue_abs) / np.array(errors_blue)
    print(coefs_blue)
    print('hfotkkfj')
    coefs_red = np.array(errors_red_abs) / np.array(errors_red)
    print(coefs_red)
    print('Coeff')
    coefs_blue = np.mean(coefs_blue, 0)
    print(coefs_blue)
    print('hfotkkfj')
    coefs_red = np.mean(coefs_red, 0)
    print(coefs_red)


    x_blue = [i for i in range(len(coefs_blue))]
    x_red = [i for i in range(len(coefs_red))]
    plt.figure()
    plt.title('Coefficient abs/acp')
    plt.plot(x_blue, coefs_blue, color='blue',label='Blue coefficient', linewidth=1, marker='o', markersize=1)
    plt.plot(x_red, coefs_red, color='red',label='Red coefficient', linewidth=1, marker='o', markersize=1)
    plt.xlabel('Wavelenght')
    plt.ylabel('abs/acp')

    # Аппроксимация
    coefs_blue_exclude_little = []
    for i in coefs_blue:
        if i < 0.01:
            i = 0.01
            coefs_blue_exclude_little.append(i)
        else:
            coefs_blue_exclude_little.append(i)

    poly_coeffs = np.polyfit(x_blue, coefs_blue_exclude_little, 4)
    ...
    # Построение
    x_plot = np.linspace(min(x_blue), max(x_blue), 1000)
    y_poly = np.array(poly(x_plot))
    plt.plot(x_plot, y_poly, color='green', label='4th polynom', linewidth=2, linestyle='dashed')


    print(f"R² = {r2_score(coefs_blue_exclude_little, y_poly):.4f}")

    plt.figure(3)
    plt.scatter(x_blue, coefs_blue_exclude_little - y_poly)
    ...
    # Аппроксимация
    poly_coeffs = np.polyfit(x_red, coefs_red, 4)
    poly = np.poly1d(poly_coeffs)


    # Построение
    x_plot = np.linspace(min(x_red), max(x_red), 1000)
    y_poly = np.array(poly(x_plot))
    plt.plot(x_plot, y_poly, color='orange', label='4th polynom', linewidth=2, linestyle='dashed')

    plt.legend()
    plt.grid()
    plt.show()

    print(f"R² = {r2_score(coefs_red, y_poly):.4f}")

    plt.figure(4)
    plt.scatter(x_red, coefs_red - y_poly)
...
def init_data_balmer_Hb_Db_gauss(chosen_time, data_directory_1, data_directory_2):
    ...
    instrumental_error = [0.05 * i for i in y_region] #взять из стд файла где мерили 1000 раз
    a_sigma = True
...
def main():
    ...
    data_directory_1 = r'C:\Users\elena\PycharmProjects\PythonProject\.venv\FTI_work\avantes\data\111225\p40 45.STR8'
    #data_directory_1 = r'C:\Users\elena\PycharmProjects\PythonProject\.venv\FTI_work\avantes\260518\Noise blue absolute.STR8'
    data_directory_2 = r'C:\Users\elena\PycharmProjects\PythonProject\.venv\FTI_work\avantes\data\111225\p40 66.STR8'
    ...
    #init_graph_autopeaking(chosen_time, i_filter, data_directory_1, data_directory_2)
    ...
    # data_directory_3 = r'C:\Users\elena\PycharmProjects\PythonProject\.venv\FTI_work\avantes\data\260518\blue_noise_abs.csv'
    # data_directory_4 = r'C:\Users\elena\PycharmProjects\PythonProject\.venv\FTI_work\avantes\data\260518\red_noise_abs.csv'
    # coef_absolute_to_acp(data_directory_1, data_directory_2, data_directory_3, data_directory_4)
