...
def gauss_Hb_big(x, A1, d_lambda, sigma, A0):
    return  A1 * np.exp(-(x - 486.269 - d_lambda)**2 / (2 * sigma**2)) + A0

def gauss_Db_big(x, A1, d_lambda, sigma, A0):
    return  A1 * np.exp(-(x - 486.137 - d_lambda)**2 / (2 * sigma**2)) + A0

def gauss_Hb_small(x, A1, d_lambda, sigma, A0):
    return  A1 * np.exp(-(x - 486.269 - d_lambda)**2 / (2 * sigma**2)) + A0
...
def balmer_Hb_Db_gauss(x, A1, A2, A3, A4, d_lambda, sigma_Hb, sigma_Db, sigma_Hs, sigma_Ds, A0):
    y_res_balmer_Hb_Db_gauss = A1 * np.exp(-(x - 486.269 - d_lambda)**2 / (2 * sigma_Hb**2)) +\
                                A2 * np.exp(-(x - 486.137 - d_lambda)**2 / (2 * sigma_Db**2)) + \
                                A3 * np.exp(-(x - 486.269 - d_lambda) ** 2 / (2 * sigma_Hs ** 2)) + \
...
def init_data_balmer_Hb_Db_gauss(chosen_time, data_directory_1, data_directory_2):
    ...
    A1, A2, A3, A4, d_lambda, sigma_Hb, sigma_Db, sigma_Hs, sigma_Ds, A0 = 35820, 42480, 0, 0, 0, 0.1, 0.1, 0.5, 0.5, 0


    popt_full, pcov_full = curve_fit(
        balmer_Hb_Db_gauss, x_region, y_region,
        p0=[A1, A2, A3, A4, d_lambda, sigma_Hb, sigma_Db, sigma_Hs, sigma_Ds, A0],
        bounds=([0, 0, 0, 0, -100000, 0, 0, 0, 0, 0], [100000, 100000, 100000, 100000, 1, 100000, 100000, 100000, 100000, 100000] )
    )


    # A1, sigma, d_lambda, A0 = 63460, 0.1, 0, 0
    #
    # popt_full_1, pcov_full_1 = curve_fit(
    #     gauss_Db_big, x_region, y_region,
    #     p0=[A1, sigma, d_lambda, A0]
    # )
    #
    # A1, sigma, d_lambda, A0 = 5000, 0.1, 0, 0
    #
    # popt_full_2, pcov_full_2 = curve_fit(
    #     gauss_Hb_big, x_region, y_region,
    #     p0=[A1, sigma, d_lambda, A0]
    # )
    #
    # A1, sigma, d_lambda, A0 = 0, 0.1, 0, 0
    #
    # popt_full_3, pcov_full_3 = curve_fit(
    #     gauss_Db_small, x_region, y_region,
    #     p0=[A1, sigma, d_lambda, A0]
    # )
    #
    # A1, sigma, d_lambda, A0 = 0, 0.1, 0, 0
    #
    # popt_full_4, pcov_full_4 = curve_fit(
    #     gauss_Hb_small, x_region, y_region,
    #     p0=[A1, sigma, d_lambda, A0]
    # )
